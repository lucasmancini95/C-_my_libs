...
//Implementación en C++11 de los tres algoritmos estudiados en clase para calcular coeficientes binomiales
...
uint C_mem(uint n, uint k, cache_t *cache=NULL)
{
    ...
    uint c;
    auto cached = cache->find({n,k});

    if(k == 0 || n == k)
        c = 1;
    else if(cached != cache->end())
        c = cached->second;
    else
    {
