...
/***************************************************************/
/*********Funciones correspondientes a "command line"***********/
static void
...
FourierTransform* build_fourier_transformation_type(FourierTransform_t  type){
	FourierTransform * aux;
...
//La siguiente función destruye el objeto que se creo anteriormente,ya sea para el foward o el reverse
void destroy_fourier_transformation_type(FourierTransform* aux){delete aux;}
...
void process_signal(FourierTransform *pFT, FourierTransform *pFT_inv){
	...
	bool is_EOF=false, last_block = true;
	...
	for(i=0;i<(size_t)block_shift;i++){

		if(taps==0 ){
			if(i<(size_t)block_shift)//se rellena el vector con 1
				for (; i < (size_t)block_shift; i++)
					Taps[i] = 1;
		break;
		}
 		if(!(*taps >> aux ))
			cerr << "Error, not enough coeficients in \"taps\" file " << endl;
	...
	while (is_EOF == false){
		...
		if(i==0){break;}//Si no hay nuevos elementos, se sale del bucle "while"

		if(is_EOF == false || last_block == true){
...
//		for(size_t i=0;i<(size_t)block_shift;i++) //Si se quisiera visualizar lo que hace la transformada
//			cout << Xout[i] << endl;
...
int main(int argc, char * const argv[]){
  cmdline cmdl(options);
	...
	p_FT = build_fourier_transformation_type(fourier_transformation_type);
	p_FT_inv = build_fourier_transformation_type(fourier_transformation_type_inv);

	process_signal(p_FT, p_FT_inv);

	destroy_fourier_transformation_type(p_FT);
	destroy_fourier_transformation_type(p_FT_inv);
